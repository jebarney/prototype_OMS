...
root.title("Seguimiento de Pedidos(ADMIN)")
root.iconbitmap("./images/bulbo.ico")
...
class login_page:
    def __init__(self, top=None):
        ...
        top.resizable(0, 0)
        top.title("Seguimiento de Pedidos(ADMIN)")
    ...
    def login(self, Event=None):
        ...
        if results:
            if results[0][6]=="Admin":
                # messagebox.showinfo("Pagina Login", "El login fue exitoso.")
                ...
                messagebox.showerror("Oops!!", "No eres un admin.")
...
def exitt():
    sure = messagebox.askyesno("Salida","¿Estas seguro que quieres salir?", parent=root)
    if sure == True:
        adm.destroy()
        root.destroy()
...
class Admin_Page:
    def __init__(self, top=None):
        ...
        top.iconbitmap("./images/bulbo.ico")
        ...
        self.button2.configure(text="""Pedidos""")
        ...
        self.button3.configure(text="""Guias""")
        ...
        self.button4.configure(text="""Reportes""")
        ...
        self.button5.configure(text="""Otros""")
        # self.button5.configure(command=about)

    def Logout(self):
        sure = messagebox.askyesno("Logout","¿Estas seguro que quieres cerrar sesión?", parent=adm)
        if sure == True:
            adm.destroy()
            root.deiconify()
            page1.entry1.delete(0, END)
...
class Inventory:
    def __init__(self, top=None):
        ...
        self.button3.configure(font="-family {Poppins SemiBold} -size 12")
        self.button3.configure(borderwidth="0")
        self.button3.configure(text="""AGREGAR PEDIDO""")
        ...
        self.button4.configure(font="-family {Poppins SemiBold} -size 12")
        self.button4.configure(borderwidth="0")
        self.button4.configure(text="""EDITAR PEDIDO""")
        ...
        self.button5.configure(font="-family {Poppins SemiBold} -size 12")
        self.button5.configure(borderwidth="0")
        self.button5.configure(text="""ELIMINAR PEDIDO""")
        ...
        self.tree.heading("Vendedor", text="Vendedor", anchor=W)
        self.tree.heading("Fecha Pedido", text="Fecha Pedido", anchor=W)
        self.tree.heading("# Cliente", text="# Cliente", anchor=W)
        self.tree.heading("Cliente", text="Cliente", anchor=W)
        self.tree.heading("OC Ventas", text="OC Ventas", anchor=W)
        self.tree.heading("Cajas", text="Cajas", anchor=W)
        self.tree.heading("Marca", text="Marca", anchor=W)
        self.tree.heading("Pedido Lertek", text="Pedido Lertek", anchor=W)
        self.tree.heading("No. de Pedido", text="No. de Pedido", anchor=W)
    ...
    def search_product(self):
        ...
        try:
            ...
            for search in val:
                if search==to_search:
                    self.tree.selection_set(val[val.index(search)-9])
                    self.tree.focus(val[val.index(search)-9])
    ...
    def delete_product(self):
        ...
        if len(self.sel)!=0:
            sure = messagebox.askyesno("Confirmar", "¿Estas seguro que quieres borrar este pedido?", parent=inv)
            if sure == True:
                for i in self.sel:
                    for j in self.tree.item(i)["values"]:
                        val.append(j)
                ...
                for k in to_delete:
                    delete = "DELETE FROM precios WHERE no_pedido = ?"
    ...
    def update_product(self):
        if len(self.sel)==1:
            ...
            messagebox.showerror("Error","Favor de seleccionar un pedido.", parent=inv)
        else:
            messagebox.showerror("Error","Solo se puede editar un pedido a la vez.", parent=inv)
...
class add_product:
    def __init__(self, top=None):
        ...
        self.entry5.place(relx=0.132, rely=0.763, width=374, height=30)
        ...
        self.entry9.place(relx=0.527, rely=0.763, width=374, height=30)
        ...
        self.button2.configure(borderwidth="0")
        self.button2.configure(text="""BORRAR""")
        self.button2.configure(command=self.clearr)

    def add(self):
        cliente = self.entry3.get()
        fpedido = self.entry2.get()  
        cajas = self.entry4.get()  
        vendedor = self.entry1.get()  
        ncliente = self.entry6.get()  
        ocventas = self.entry7.get()  
        marca = self.entry8.get()
        plertek = self.entry5.get()
        npedido = self.entry9.get()  
            ...
            if fpedido:
                if ncliente:
                    try:
                        ...
                        messagebox.showerror("Oops!", "# Cliente invalido.", parent=p_add)
                    else:
                        if cliente:
                            if ocventas:
                                if cajas:
                                    try:
                                        ...
                                        messagebox.showerror("Oops!", "Cajas invalido.", parent=p_add)
                                    else:
                                        if marca:
                                            if plertek:
                                                if npedido:
                                                    ...
                                                    messagebox.showerror("Oops!", "Favor de ingresar No. de Pedido", parent=p_add)
                                            else:
                                                messagebox.showerror("Oops!", "Favor de ingresar Pedido Lertek", parent=p_add)
                                        else:
                                            messagebox.showerror("Oops!", "Favor de ingresar OC Ventas", parent=p_add)
                                else:
                                    messagebox.showerror("Oops!", "Favor de ingresar Cajas", parent=p_add)
                            else:
                                messagebox.showerror("Oops!", "Favor de ingresar OC Ventas", parent=p_add)
                        else:
                            messagebox.showerror("Oops!", "Favor de ingresar Cliente", parent=p_add)
                else:
                    messagebox.showerror("Oops!", "Favor de ingresar # Cliente", parent=p_add)
            else:
                messagebox.showerror("Oops!", "Favor de ingresar Fecha Pedido", parent=p_add)
        else:
            messagebox.showerror("Oops!", "Favor de ingresar Vendedor", parent=p_add)
        ...
        #                             else:
        #                                 if valid_phone(marca):
        #                                     with sqlite3.connect("./Database/store.db") as db:
        #                                         cur = db.cursor()
        #                                     insert = (
        #                                                 "INSERT INTO raw_inventory(product_name, product_cat, product_subcat, stock, mrp, cost_price, vendor_phn) VALUES(?,?,?,?,?,?,?)"
        #                                             )
        #                                     cur.execute(insert, [vendedor, fpedido, psubcat, int(cliente), float(cajas), float(ocventas), marca])
        #                                     db.commit()
        #                                     messagebox.showinfo("Success!!", "Product successfully added in inventory.", parent=p_add)
        #                                     p_add.destroy()
        #                                     page3.tree.delete(*page3.tree.get_children())
...
class Update_Product:
    def __init__(self, top=None):
        ...
        self.button1.configure(font="-family {Poppins SemiBold} -size 14")
        self.button1.configure(borderwidth="0")
        self.button1.configure(text="""EDITAR""")
    ...
    def update(self):
            ...
            if fpedido:
                if ncliente:
                    try:
                        ...
                        messagebox.showerror("Oops!", "# Cliente invalido.", parent=p_update)
                    else:
                        if cliente:
                            if ocventas:
                                if cajas:
                                    try:
                                        ...
                                        messagebox.showerror("Oops!", "Cajas invalido.", parent=p_update)
                                    else:
                                        if marca:
                                            if plertek:
                                                if npedido:
                                                    ...
                                                    with sqlite3.connect("./Database/store.db") as db:
                                                        cur = db.cursor()
                                                    update = (
                                                    "UPDATE precios SET vendedor = ?, fecha_pedido = ?, num_cliente = ?, cliente = ?, oc_ventas = ?, cajas = ?, marca = ?, pedido_lertek = ? WHERE no_pedido = ?"
                                                    )
                                                    cur.execute(update, [vendedor, fpedido, int(ncliente), cliente, ocventas, int(cajas), marca, plertek, int(npedido)])
                                                    db.commit()
                                                    messagebox.showinfo("¡¡Exito!!", "El pedido fue editado exitosamente.", parent=p_update)
                                                    valll.clear()
                                                    Inventory.sel.clear()
                                                    page3.tree.delete(*page3.tree.get_children())
                                                    page3.DisplayData()
                                                    ...
                                                    messagebox.showerror("Oops!", "Favor de ingresar No. de Pedido", parent=p_update)
                                            else:
                                                messagebox.showerror("Oops!", "Favor de ingresar Pedido Lertek", parent=p_update)
                                        else:
                                            messagebox.showerror("Oops!", "Favor de ingresar OC Ventas", parent=p_update)
                                else:
                                    messagebox.showerror("Oops!", "Favor de ingresar Cajas", parent=p_update)
                            else:
                                messagebox.showerror("Oops!", "Favor de ingresar OC Ventas", parent=p_update)
                        else:
                            messagebox.showerror("Oops!", "Favor de ingresar Cliente", parent=p_update)
                else:
                    messagebox.showerror("Oops!", "Favor de ingresar # Cliente", parent=p_update)
            else:
                messagebox.showerror("Oops!", "Favor de ingresar Fecha Pedido", parent=p_update)
        else:
            messagebox.showerror("Oops!", "Favor de ingresar Vendedor", parent=p_update)
        ...
        #                                     )
        #                                     cur.execute(update, [pname, pcat, psubcat, int(cliente), float(pmrp), float(pcp), pvendor, product_id])
        #                                     db.commit()
